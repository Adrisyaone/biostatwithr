...
alpha <- 0.05
power <- 0.80
delta <- 2          # e.g., a 2-fold increase in median survival time (14 vs 7 months)
...
Whenever a study involves several hypothesis tests on the same data set (pairwise group comparisons, multiple outcomes, subgroup analyses), the probability of at least one **false positive** across the whole family of tests — the **familywise (experimentwise) error rate** — climbs well above the nominal per-test $\alpha$. For example, with 10 independent tests each conducted at $\alpha=0.05$, the probability of at least one Type I error is $1-(1-0.05)^{10} \approx 0.40$.
