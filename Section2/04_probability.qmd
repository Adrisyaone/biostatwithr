...
flowchart LR
    ...
    direction LR
        A_only["A only<br/>P(A ∩ Bᶜ)"] --- AB["A ∩ B<br/>(both A and B)"] --- B_only["B only<br/>P(Aᶜ ∩ B)"]
        Neither["Neither A nor B<br/>P(Aᶜ ∩ Bᶜ)"]
    end
```
...
```{mermaid}
flowchart LR
    subgraph S["Clinic Population, n = 200"]
    direction LR
        A["Diabetes only<br/>n = 60"] --- AB["Both<br/>n = 40"] --- B["Hypertension only<br/>n = 50"]
        N["Neither<br/>n = 50"]
    end
```
