...
![Fig: Basic structure of function](/Section1/Pictures/3e0c562c-f5ad-424b-bb2b-9d6e3c6d0152.png)

In the figure above, function $f(x)$ processes the input of 5 and gives output of 125. This showed that, function needs input which when processed gives an output.
...
1.  **Using the `help()` function:** The primary method to access help in R is using the **`help()`** function or its shorthand **`?`**. You simply pass the function's name or topic as an argument to **`help()`** or **`?`**, respectively.
    ...
    ![](/Section1/Pictures/picture_8.png)
...
2.  **Search for Keywords:** You can also use the **`help.search()`** function to search for keywords across all available help pages. This function will list all the topics that match the provided keywords.
    ...
    ![](/Section1/Pictures/picture_10.png)
...
![](/Section1/Pictures/steps%20to%20import%20data.png)
...
dt[,3] # select third column and all rows
dt[, c(2,4,5)] # select second, fourth and fifth column and all rows

# Access specific row by row index
dt[1, ] # filter first row and all column
dt[1:30, ]  # filter row 1 to 30 and all column
dt[c(1,4,6,15:20), ] # filter row 1, 4,6 and 15 to 20 and  all column


# Access specific cell
dt[3,2] # select value from a cell referring to 3rd row and 2nd column
```
